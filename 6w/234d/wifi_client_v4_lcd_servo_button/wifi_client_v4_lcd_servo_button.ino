...
char sendId[10] = "KJY_ARD";
char sendBuf[CMD_SIZE];
char lcdLine1[17] = "Smart IoT By KJY";
...
*/
    ...
    if (sensorTime != 0 && !(secCount % sensorTime)) {  //what is this condition?
      char tempStr[5];
      char humiStr[5];
      dtostrf(humi, 4, 1, humiStr);  //50.0   4:전체자리수,1:소수이하 자리수
      dtostrf(temp, 4, 1, tempStr);  //25.1
...
void socketEvent() {
  ...
  } else if (!strncmp(pArray[1], " Alr", 4))  //Already logged
  {
    ...
    client.write(sendBuf, strlen(sendBuf));
    client.flush();
    sprintf(sendBuf, "[%s]SETDB@%s@%s\n", "KJY_SQL", pArray[1], pArray[2]);
